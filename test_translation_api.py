...
from md_sync.translate.fallback import translate_via_api, _detect_provider
...
def test_provider(provider_name: str, test_cases: list[tuple[str, str]]) -> dict:
    ...
    results = {"provider": provider_name, "success": 0, "failed": 0, "times": [], "translations": []}
    
    for zh_text, expected_en in test_cases:
        start = time.perf_counter()
        result = translate_via_api(zh_text, provider=provider_name, source_lang="zh", target_lang="en")
...
def main():
    ...
    for r in all_results:
        if "error" in r:
            ...
            print(f"{r['provider']}: 成功 {r['success']}/{r['success']+r['failed']}, 平均响应 {avg_time:.1f}ms")
